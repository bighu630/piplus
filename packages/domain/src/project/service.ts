...
export async function createProjectWithPlanner(db: RoleManagerDb, piClient: PiClient, name: string, createdBy: string) {
  const roleManager = createRoleManagerService(db, piClient);
  return roleManager.createProjectWithPlanner({ name, createdBy });
}
