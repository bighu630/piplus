...
function compilePrompt(input: {
  ...
  if (input.userSuppliedPrompt) {
    parts.push(input.userSuppliedPrompt);
  }
...
export function createRoleManagerService(db: RoleManagerDb, piClient: PiClient) {
  return {
    ...
    async createTopLevelPlannerSession(input: { projectId: string; createdBy: string; plannerTemplate?: SessionTemplateRow }) {
      const plannerTemplate = input.plannerTemplate ?? await findRoleTemplate(db, 'planner');
      const sessionId = id('session');
      const title = `Planner - ${input.projectId}`;
      ...
      return { sessionId, piSessionId: piSession.sessionId };
    },
    ...
    async createTopLevelBlankSession(input: CreateSessionInput) {
      ...
      await touchProject(db, input.projectId);
      return { projectId: input.projectId, sessionId, piSessionId: piSession.sessionId };
    },
    ...
    async spawnSession(input: SpawnSessionInput) {
      ...
      const title = `${template.name} - ${input.target}`;
      const compiledPrompt = compilePrompt({
        roleBasePrompt: template.basePrompt,
        userSuppliedPrompt: input.target,
