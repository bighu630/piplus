...
#
# 用法：
#   bash scripts/setup-hooks.sh          安装（已安装则直接返回；非 git 环境跳过，exit 0）
...
# hooks 只在 git 仓库里有意义。非 git 环境（Docker 构建上下文、源码 tarball、
# 去掉 .git 的 CI checkout）不是错误：本脚本由 package.json 的 prepare 调用，
# 此时 exit 1 会让 `bun install` 直接失败（实测 Dockerfile 构建挂在第 5 步）。
# 安装模式（含 --force）打印提示后 exit 0；--check 是显式校验，仍需 exit 1。
...
if [ -z "$ROOT" ]; then
  if [ "$MODE" = "check" ]; then
    echo "❌ setup-hooks: 不在 git 仓库内，无法检查 hooks 安装状态" >&2
    exit 1
  fi
  echo "setup-hooks: 不在 git 仓库内，跳过 hooks 安装（非 git 环境无需 hooks）"
  exit 0
fi
cd "$ROOT"
