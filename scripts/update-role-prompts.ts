/**
 * 将 init.ts 中最新的角色提示词重新注入到已存在的数据库中。
 * 使用方式：bun run scripts/update-role-prompts.ts
 */
...
function main() {
  ...
  const upsertStmt = db.prepare(`
    INSERT INTO role_templates (id, key, version, name, description, base_prompt, config_json, created_by, owner_type, visibility, is_builtin, created_at, updated_at)
    VALUES (?, ?, ?, ?, ?, ?, '{}', NULL, 'system', 'public', 1, ?, ?)
  ...
  for (const role of ROLES) {
    ...
    upsertStmt.run(id, role.key, role.version, role.name, role.description, role.basePrompt, now, now);
    console.log(`  ✓ ${id} (${role.name}) → v${role.version}`);
    inserted++;
  }

  db.close();
  console.log(`\n[update-role-prompts] 完成。已更新 ${inserted} 个角色模板。`);
}
